...
class Col{
   ...
   /** @type {'number' | 'varchar' | 'date' | 'fk' | 'm2m'} */ data_type
   /** @type {Fk} */ fk

   /** @param {import("common/col").DbCol} col_data */
   constructor(col_data){
      ...
      this.data_type = defineDataType(col_data.data_type, col_data.udt_name)
   }
}
 ...
 * @returns {'number' | 'varchar' | 'date'}
 */
function defineDataType(data_type, udt_name){
   if (data_type.includes('timestamp')) return 'date'
   if (udt_name.includes('int') || udt_name.includes('float') || udt_name.includes('numeric') ||
         data_type.includes('real') || data_type.includes('numeric')){
      return 'number'
   }
   return 'varchar'
}
