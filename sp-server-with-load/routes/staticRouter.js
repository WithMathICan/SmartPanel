...
function ErrorWrapper(func) {
   async function router(method, url) {
      assert(method.toUpperCase() === 'GET', 'Static method should be GET! URL: ' + url)
      try {
...
function createStaticRouter(public_root) {
   return ErrorWrapper(async function (url) {
      const filePath = await findFilePath(public_root, url)
...
function createIndexHtmlRouter(public_root, sp_name) {
   return ErrorWrapper(async function IndexHtml(url) {
