...
pool.query('SELECT 1+1').then(async () => {
   ...
   const staticHandler = createStaticHandler(publicRoot, logger)
   const indexHtmlHandler = createIndexHtmlHandler(publicRoot, config.SP_NAME)
   ...
   http.createServer(async (req, res) => {
      ...
      const args = method === 'POST' ? await receiveArgs(req) : {}
      ...
      if (!resData) {
         const handler = apiRouter(method, url)
         if (handler) resData = await handler({ ...args })
      }

      const resIsJson = req.headers['accept'] === 'application/json'
      if (resData) {
